...
class WikiProvider {
	...
	downloadWiki() {
		console.log("vscode-laux: downloading gmod wiki");

		this.LAUX.downloadingMsg = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left);
		this.LAUX.downloadingMsg.text = "$(cloud-download) Downloading Gmod Wiki";
		this.LAUX.downloadingMsg.show();
		...
		https.get('https://venner.io/gmod-wiki.json', stream => {
			...
			stream.on('end', () => {
				...
				if (data) {
					this.LAUX.extension.globalState.update("vscode-laux-enhanced-wiki-date", Math.round(new Date().getTime()));
					this.LAUX.extension.globalState.update("vscode-laux-enhanced-wiki-data", data);
					...
					this.LAUX.CompletionProvider.createCompletionItems();

					console.log("vscode-laux: gmod wiki downloaded and ingested");
				}

				if (this.LAUX.downloadingMsg) {
					this.LAUX.downloadingMsg.dispose();
					this.LAUX.downloadingMsg = undefined;
				}
			});
		});
	}
	...
	getRealmIcon(client, menu, server) {
		...
		if (!(realm_icon_id in this.realmIcons)) {
			let full_path = "file:///" + this.LAUX.extension.asAbsolutePath("resources/icons/realm_" + realm_icon_id + ".svg");
	...
	getLabelIcon(label) {
		...
		if (!(label in this.labelIcons)) {
			let full_path = "file:///" + this.LAUX.extension.asAbsolutePath("resources/icons/label_" + label + ".svg");
	...
	resolveCompletionItem(item, cancel) {
		...
		if (doc) {
			...
			if ("VMT" in doc) {
				return this.LAUX.VMTProvider.provideVMT(cancel, item, doc["VMT"]);
			}
			...
			console.warn("vscode-laux couldn't find \"" + item.label + "\" in wiki docs!");
		}
	}
	...
	initWiki() {
		...
		const storedWikiData = this.LAUX.extension.globalState.get("vscode-laux-enhanced-wiki-data");
