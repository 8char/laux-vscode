...
class HoverProvider {
	constructor(LAUX) {
		this.LAUX = LAUX;
		this.LAUX.HoverProvider = this;

		this.registerSubscriptions();
	}
	
	registerSubscriptions() {
		this.LAUX.extension.subscriptions.push(vscode.languages.registerHoverProvider("laux", { provideHover: (document, pos, cancel) => this.provideWikiHover(this, document, pos, cancel) }));
		// this.LAUX.extension.subscriptions.push(vscode.languages.registerHoverProvider("laux", { provideHover: (document, pos, cancel) => this.provideStringHover(this, document, pos, cancel) }));
	}

	getHoverCallExpression(document, pos) {
		let token = this.LAUX.LAUXParser.getTokenAt(document, pos);
		while (token) {
			if (token.type === "CallExpression") {
				...
				if (!token.isLocal) {
					...
					if (tag in this.LAUX.WikiProvider.docs) {
		...
		for (let i = 0; i < docs.length; i++) {
			...
			hovers.push(new vscode.MarkdownString().appendCodeblock(
				...
				(doc["TAG"] === "ENUM" ? ("(enum) " + doc["SEARCH"]) :
					(doc["SEARCH"] + "(" +
						("ARGUMENTS" in doc ? this.LAUX.SignatureProvider.generateSignatureString(doc["ARGUMENTS"]) : "")
					+ ")")
				)
			, "laux"));

			hovers.push(this.LAUX.WikiProvider.resolveDocumentation(doc));
		}
	}
	...
	provideWikiHover(HoverProvider, document, pos, cancel) {
		...
		if (token) {
			...
			switch(hover_type) {
				case HOVER_GENERIC:
					...
					if (callExpression) {
						...
						if (full_call.length >= 2 && full_call[full_call.length-2] === ":") {
							...
							if (meta_func in this.LAUX.TokenIntellisenseProvider.compiledTokenData.signatures.metaFunctions) {
								this.pushWikiHovers(hovers, this.LAUX.TokenIntellisenseProvider.compiledTokenData.signatures.metaFunctions[meta_func]);
							}
						}
					}
				...
				case HOVER_ENUM:
					this.pushWikiHovers(hovers, [this.LAUX.WikiProvider.docs[full_call]]);
					break;
			}
		...
		while ((match = REGEXP_ASCII_HOVER.exec(line.text)) !== null) {
			try {
				...
				if (ascii_range.contains(pos)) {
					...
					return new vscode.Hover(new vscode.MarkdownString().appendCodeblock(bytes, "laux"), ascii_range);
				}
			} catch(e) {}
		}
